...
class MinesController extends Controller
{
    ...
    public function resolvePath(Request $r)
    {
        ...
        if($setting->antiminus == 1 && !$game['bombs'][$r->bomb] && !$user->is_youtuber || $user->is_loser) { // Антиминус
            $game['used_positions'][$r->bomb] = 0;
            $user->update([
                'mines_game' => json_encode($game)
            ]);
            $totalwin = $this->getCoef($user, true) * $game['bet'] - $game['bet'];
            Telegram::create(['message' => $totalwin . ' Банк:' . $profit->bank_mines]);
    ...
    public function resolveTake(Request $r)
    {
       ...
       try {
           ...
           $affectedRow = \App\User::where([['mines_is_active', '=', 1], ['id', $user->id]])->update([
               'mines_is_active' => 0
           ]);
           ...
           if (!$affectedRow) {
		    ...
		    /*if ($user->mines_is_active == 0) {
                return ['success' => false, 'message' => 'У вас нет активной игры'];
            }*/ 

            if($this->tooFastNew($user->id)) return ['success' => false, 'message' => 'Не спешите'];

		  //$user->update(['mines_is_active' => 0]);
            $winSum = $this->cashout($user);
